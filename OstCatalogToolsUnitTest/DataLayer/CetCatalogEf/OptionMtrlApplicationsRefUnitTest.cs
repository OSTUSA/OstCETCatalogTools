...
{
    public class OptionMtrlApplicationsRefUnitTest : CatalogBaseUnitTest
    {
        [Fact]
        public async Task ToListAsync_Void_NotEmpty()
        {
            var list = await CetCatalogContext.OptionMtrlApplicationsRefs.ToListAsync();
        ...
        {
            var first = await CetCatalogContext.OptionMtrlApplicationsRefs
                .Include(r => r.Owner)
                .Include(r => r.Child)
                .FirstOrDefaultAsync();
