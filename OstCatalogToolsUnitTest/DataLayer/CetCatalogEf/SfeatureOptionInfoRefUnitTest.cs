...
{
    public class SfeatureOptionInfoRefUnitTest : CatalogBaseUnitTest
    {
        [Fact]
        public async Task ToListAsync_Void_NotEmpty()
        {
            var list = await CetCatalogContext.SfeatureOptionInfoRefs.ToListAsync();
        ...
        {
            var first = await CetCatalogContext.SfeatureOptionInfoRefs
                .Include(f => f.Owner)
                .Include(f => f.Child)
                .FirstOrDefaultAsync();
